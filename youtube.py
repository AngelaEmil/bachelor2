            ...
            # if (results[0].numpy().boxes[r].cls) == 2:
                ...
                if (class_id==2 or class_id==3 or class_id==5 or class_id==7):
                    ...
                    for track in tracker.tracks:
                        bbox=track.bbox
                        x11,y11,x21,y21=bbox
                        track_id=track.track_id
                        # print(track_id)
                        cv2.rectangle(frame,(int(x11),int(y11)),(int(x21),int(y21)),(colors[track_id%len(colors)]),3)
                        cv2.putText(frame, f'ID:{track_id}', (int(x11),int(y11)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)
